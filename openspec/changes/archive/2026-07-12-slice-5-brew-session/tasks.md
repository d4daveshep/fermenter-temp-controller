...
- [x] 1.1 Create `src/brew_session.rs` module (mirrors
      `temperature_control.rs`'s shape per design.md decision 5); register it
      in `src/lib.rs`
- [x] 1.2 Write unit tests (RED, `rstest` cases) for a pure validation
...
- [x] 1.3 Implement `validate_brew_id` (GREEN for 1.2)
- [x] 1.4 Write a test (RED, using `FakeTimeStore`) for
      `initial_brew_id(store, default) -> String`: returns the persisted
      `ControllerState.brew_id` when `load_state()` returns `Some`
- [x] 1.5 Write a test (RED): `initial_brew_id` returns the passed-in
      `default` when `load_state()` returns `None`
- [x] 1.6 Implement `initial_brew_id` (GREEN for 1.4/1.5)
- [x] 1.7 Write a test (RED) confirming `persist_brew(store, brew_id,
...
- [x] 1.8 Implement `persist_brew` (GREEN for 1.7)

## 2. Ingest loop: dynamic brew tagging — TDD

- [x] 2.1 Write a test (RED) in `ingest.rs`: given a `MockSerialSource` with
...
- [x] 2.2 Write a test (RED): when the brew identifier changes between two
      ...
      one
- [x] 2.3 Change `ingest_loop`'s signature: replace `brew_id: &str` with
      ...
      2.1/2.2)
- [x] 2.4 Update all existing `ingest.rs` call sites (slice-1–4 tests) to
...
- [x] 3.1 Replace `web::AppState.brew_id: String` with
...
- [x] 3.2 Update `StatusContext`/`status_context` (`web/handlers.rs`) to read
      `state.brew_tx.borrow().clone()` instead of the removed `brew_id`
      field
- [x] 3.3 Create `templates/brew_form.html` (`{% extends "base.html" %}`),
...
- [x] 3.4 Add a link/control from `dashboard.html` to `GET /brew` (alongside
      the existing `/target` link)
- [x] 3.5 Write an `insta` snapshot test (RED) rendering `brew_form.html`
      with a seeded current-brew context and no error
- [x] 3.6 Write an `insta` snapshot test (RED) rendering `brew_form.html`
      with an error message present
- [x] 3.7 Implement the `/brew` `GET` handler (`brew_form`): builds a
      context from `AppState.brew_tx.borrow()`'s current value, renders
      `brew_form.html` (GREEN for 3.5/3.6)
- [x] 3.8 Write a `tower::oneshot` HTTP test (RED): `POST /brew` with a
...
- [x] 3.9 Write a `tower::oneshot` HTTP test (RED): `POST /brew` with an
      empty value returns the redisplayed form with an error, and
      `brew_tx`'s value is unchanged
- [x] 3.10 Write a `tower::oneshot` HTTP test (RED): `POST /brew` with a
      whitespace-only value returns the redisplayed form with an error, and
      `brew_tx`'s value is unchanged
- [x] 3.11 Write a `tower::oneshot` HTTP test (RED) for design.md decision 4
...
- [x] 3.12 Write a `tower::oneshot` HTTP test (RED): `POST /brew` switching
...
- [x] 3.13 Implement the `/brew` `POST` handler (`set_brew`): parse the form
...
- [x] 3.14 Wire `GET /brew` and `POST /brew` routes into `build_router`

## 4. Persistence symmetry for `/target` (resolves slice-4 open question)

- [x] 4.1 Write a test (RED) confirming `set_target`'s handler now persists
...
- [x] 4.2 Update `web/handlers.rs`'s `set_target` handler's call to
      `persist_target` to pass `&state.brew_tx.borrow().clone()` in place of
      the removed `&state.brew_id` (GREEN for 4.1)
- [x] 4.3 Confirm existing `/target` tests (slice-4) still pass unchanged
      given the new `AppState` shape

## 5. Main wiring

- [x] 5.1 In `main.rs`, after building the store, call
...
- [x] 5.2 Update the startup call to `ingest::rehydrate_latest_reading` to
      use the resolved `initial_brew_id` instead of `config.default_brew_id`
      directly (design.md decision 5's correctness fix)
- [x] 5.3 Pass `brew_rx` into the ingest task's call to `ingest_loop` (task
      2's new signature), replacing the `&config.default_brew_id` argument
- [x] 5.4 Add `brew_tx` (cloned/wrapped as needed) to the `AppState`
      constructed for the web router, replacing the removed `brew_id` field
- [x] 5.5 Confirm `MOCK_SERIAL=true` end-to-end: submit a new brew id via
...
- [x] 6.1 Run the binary with `MOCK_SERIAL=true` against a real `redis:8`
...
- [x] 6.2 Confirm: `POST /brew` with an empty or whitespace-only value
      returns an error message; a following `GET /brew` still shows the
      previous value unchanged
- [x] 6.3 Confirm rehydration: with the process running and a reading
...
- [x] 6.4 Kill and restart the process against the same Redis container:
...
- [x] 6.5 Run `cargo fmt --check` and `cargo clippy --all-targets -- -D
      warnings` — confirm clean
- [x] 6.6 Run the full `cargo test` suite (unit + `insta` snapshots +
      `testcontainers`-backed Redis integration tests) — confirm all passing;
      review and accept any new `.snap.new` snapshot files
- [x] 6.7 Run `openspec validate slice-5-brew-session --strict` — confirm
      valid
